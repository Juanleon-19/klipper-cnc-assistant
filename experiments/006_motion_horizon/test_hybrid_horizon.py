...
async def wait_for_final_state(
    ...
    while True:
        ...
        if (
            ...
            raise TimeoutError(
                "Machine motion did not complete "
                "within timeout"
            )
...
async def run_hybrid_control(
    ...
    while active_event.is_set():
        ...
        if new_sample:
            ...
            print(
                f"[SAMPLE {telemetry_sample_count:02d}] "
                f"X={state[TEST_AXIS]:.3f} "
                f"V={state['velocity']:.3f}"
            )

        estimated_position = (
            estimator.estimated_position()
        )
        ...
        if horizon.needs_extension(
            ...
            if abs(extension) > 1e-9:
                ...
                print(
                    ...
                    f"estimated="
                    f"{estimated_position:.3f} "
                    f"distance="
                    f"{effective_distance:.3f} "
                    f"queued="
                    f"{queued_time * 1000:.1f} ms "
                    f"planned="
                    f"{horizon.planned_position():.3f}"
                )
...
async def main():
    ...
    print("HYBRID ESTIMATED MOTION HORIZON TEST")
    print("=" * 60)
    ...
    total_displacement = (
        final_state[TEST_AXIS]
        - initial_state[TEST_AXIS]
    )

    planned_position = (
        horizon.planned_position()
    )
    ...
    print(
        f"Commands sent: "
        f"{controller_result['command_count']}"
    )

    print(
        f"Telemetry samples observed: "
        f"{controller_result['telemetry_sample_count']}"
    )
