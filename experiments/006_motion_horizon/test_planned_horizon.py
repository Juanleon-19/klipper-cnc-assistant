        ...
        if horizon.needs_extension(
            ...
            if abs(extension) > 1e-9:
                ...
                print(
                    ...
                    f"observed="
                    f"{observed_position:.3f} "
                    f"distance="
                    f"{effective_distance:.3f} "
                    f"planned="
                    f"{horizon.get_planned_position(TEST_AXIS):.3f}"
                )
...
async def main():
    ...
    print("PLANNED MOTION HORIZON TEST")
    print("=" * 60)
    ...
    settled_time = await wait_until_settled(
        machine
    )
