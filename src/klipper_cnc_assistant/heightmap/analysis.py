...
ALGORITHM_VERSION = "heightmap-v1"
...
def fit_plane(samples: list[HeightSample]) -> PlaneFit | None:
    valid = [
        ...
        if sample.incluida and sample.z_mm is not None
    ]
    if len(valid) < 3:
        return None

    matrix = numpy.array(
        [[sample.x_mm, sample.y_mm, 1.0] for sample in valid],
        dtype=float,
    )
...
def build_statistics(
    ...
    valid_samples = [sample for sample in samples if sample.z_mm is not None]
    included_samples = [
        sample for sample in samples if sample.incluida and sample.z_mm is not None
    ]
    ...
    return HeightMapStatistics(
        ...
        cantidad_puntos_atipicos=sum(
            1 for sample in samples if sample.estado_calidad == SampleQuality.ATIPICA
        ),
        ...
        rms_residuos_mm=plane.rms_residuos_mm if plane else None,
...
def interpolate_height(
    ...
    *,
    ...
    if x_mm < 0 or y_mm < 0 or x_mm > grid.ancho_mm or y_mm > grid.alto_mm:
        return InterpolationResult(
            estado="fuera de dominio",
            valor_mm=None,
            observacion="La coordenada solicitada cae fuera del material.",
        )
    ...
    column_position = (
        (x_mm / grid.paso_x_mm) if grid.paso_x_mm > 0 else 0.0
    )
    row_position = (
        (y_mm / grid.paso_y_mm) if grid.paso_y_mm > 0 else 0.0
    )
...
def build_dense_surface(
    ...
    *,
    ...
    for row in range(rows):
        y_mm = 0.0 if rows == 1 else height_map.grid.alto_mm * row / (rows - 1)
        for column in range(columns):
            x_mm = 0.0 if columns == 1 else height_map.grid.ancho_mm * column / (columns - 1)
    ...
    for sample in samples:
        ...
        if not sample.incluida:
            provisional.append(
                replace(
                    sample,
                    estado_calidad=SampleQuality.EXCLUIDA,
                    residuo_plano_mm=None,
                )
            )
            continue
